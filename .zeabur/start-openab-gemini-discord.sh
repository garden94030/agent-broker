#!/bin/sh
# Mounted at /opt/start-openab.sh in the Gemini agent service on Zeabur.
# Gemini Discord bot - fast query + archiving.
set -e
...
if [ -n "$missing" ]; then
  echo "openab-gemini: missing required env vars:$missing"
  exec sleep infinity
fi

if ! command -v python3 >/dev/null 2>&1 || ! command -v git >/dev/null 2>&1; then
  echo "openab-gemini: installing python3 + git"
  ...
  apt-get install -y --no-install-recommends python3 python3-pip git ca-certificates >/dev/null 2>&1 \
    ...
    || echo "openab-gemini: WARN failed to install python3/git"
fi
...
python3 -m pip install --quiet --break-system-packages youtube-transcript-api 2>/dev/null \
  ...
  || echo "openab-gemini: WARN youtube-transcript-api install failed"
...
clone_or_refresh "$REPO_PLA_DIR" "$GITHUB_REPO_PLA" || echo "openab-gemini: WARN PLA clone failed; continuing"
...
install_secret_hook() {
  ...
  if [ -d "$d/.git" ]; then
    ...
    echo "openab-gemini: installed secret-scan hook in $d"
  fi
}
...
echo "openab-gemini: ready (Discord + Gemini Flash Lite)"
